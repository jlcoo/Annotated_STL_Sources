...
// 每个Adaptable Unary Function都应该继承此类型
...
// 每个Adaptable Binary Function都应该继承此类
...
// negate对象继承了unary_function; 取相反数
template <class _Tp>
...
// 辅助函数, 使我们得以方便使用unary_negate<_Predicate> 
template <class _Predicate>
...
// 辅助函数，让我们得以方便使用binder1st<_Operation> 
...
public:
  ...
  operator()(const typename _Operation::first_argument_type& __x) const {
    return op(__x, value);                                        //实际调用表达式，并将value绑定为第二个参数
  }
};
...
// 以下配接器实际就是把一个二元函数指针包起来
// 当仿函数被使用时, 就调用该函数指针
template <class _Arg1, class _Arg2, class _Result>
class pointer_to_binary_function : 
  public binary_function<_Arg1,_Arg2,_Result> {
protected:
    _Result (*_M_ptr)(_Arg1, _Arg2);            //内部成员，实际上就是一个函数指针
public:
    pointer_to_binary_function() {}             //以构造函数将函数指针记录于内部成员之中
    explicit pointer_to_binary_function(_Result (*__x)(_Arg1, _Arg2)) 
      : _M_ptr(__x) {}
    _Result operator()(_Arg1 __x, _Arg2 __y) const { //通过函数指针执行函数
      return _M_ptr(__x, __y);
    }
};
// 辅助函数，让我们得以方便运用pointer_to_binary_function
template <class _Arg1, class _Arg2, class _Result>
inline pointer_to_binary_function<_Arg1,_Arg2,_Result>    //这个是ptr_fun函数的返回类型
...
// 函数对象配接器: 指向成员函数，一共有16个，4种， 
// There are a total of 16 = 2^4 function objects in this family.  
//  (1) Member functions taking no arguments vs member functions taking
//       one argument. 成员函数无参或成员函数只有一个参数
//  (2) Call through pointer vs call through reference. 通过指针或者引用调用
//  (3) Member function with void return type vs member function with
//      non-void return type.  成员函数无返回或有返回
//  (4) Const vs non-const member function. 成员函数是静态变量或非静态变量
