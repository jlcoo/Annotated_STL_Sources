...
template <class _Value,
          ...
          class _Alloc =  __STL_DEFAULT_ALLOCATOR(_Value) >               //缺省了空间配置器
class hash_set;
...
private:  //以下使用的_Identity<_Value>定义与stl_function中
  typedef hashtable<_Value, _Value, _HashFcn, _Identity<_Value>, 
                    _EqualKey, _Alloc> _Ht;   //hash_set的底层由hashtable完成
  _Ht _M_ht;
...
public:
  hash_set()                                //缺省使用大小为100的表格,将被hash table调整为最接近且较大的质数
  ...
  hash_set(size_type __n, const hasher& __hf, const key_equal& __eql,
    ...
    : _M_ht(__n, __hf, __eql, __a) {}
                                            //以下插入操作全部使用 insert_unique()函数，不允许键值重复
...
// 几乎所有的操作函数hashtable都有对应版本，传递调用就行
public:
...
// hash_multiset采用insert_equal()函数，允许键值重复
template <class _Value, class _HashFcn, class _EqualKey, class _Alloc>
class hash_multiset
{
  // requirements:
...
public:
  hash_multiset() //缺省使用大小为100的表格,将被hash table调整为最接近且较大的质数
    : _M_ht(100, hasher(), key_equal(), allocator_type()) {}
  ...
  hash_multiset(size_type __n, const hasher& __hf, const key_equal& __eql,
                const allocator_type& __a = allocator_type())
    : _M_ht(__n, __hf, __eql, __a) {}
                //以下插入操作全部使用 insert_equal()函数，允许键值重复
#ifdef __STL_MEMBER_TEMPLATES
  template <class _InputIterator>
...
public:     // 几乎所有的操作函数hashtable都有对应版本，传递调用就行
  size_type size() const { return _M_ht.size(); }
  size_type max_size() const { return _M_ht.max_size(); }
  bool empty() const { return _M_ht.empty(); }
