...
{
  {
    ...
    copy(ia + 2, ia + 7, ia);   //输出区间的终点与输入区间重叠，没问题
  ...
  {   //利用有效作用域
    int ia[] = {0, 1, 2, 3, 4, 5, 6, 7, 8};
    copy(ia + 2, ia + 7, ia + 4); //输出区间的起点与输入区间重叠，可能会有问题
...

  {
    ...
    copy(first, last, result);//输出区间的终点与输入区间重叠，没问题
...

  {
    ...
    copy(first, last, result);                          //输出区间的起点与输入区间重叠，可能会有问题
