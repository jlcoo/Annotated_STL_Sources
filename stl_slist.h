...
struct _Slist_iterator : public _Slist_iterator_base
{
  ...
  // 调用slist<T>::end()时会造成__slist_iterator(0),于是调用下述函数
...
protected:
  _Slist_node_base _M_head;     //头部，它不是指针，是实物
};  
...
public:
  ...
  typedef typename _Base::allocator_type allocator_type;
  allocator_type get_allocator() const { return _Base::get_allocator(); }//配置空间
...
public:
  ...
  void swap(slist& __x)  //两个slist互换，只要将head交换即可
...
public:
  ...
  void push_front(const value_type& __x)   {             //从头部插入元素（新元素成为slist的第一个元素）
    __slist_make_link(&this->_M_head, _M_create_node(__x));
  }     //注意没有push_back成员函数
  void push_front() { __slist_make_link(&this->_M_head, _M_create_node()); }
  void pop_front() {                                    //从头部取走元素（删除之），修改head
